...
def menus(context, _get_config=None):
    ...
    for app in app_list:
        _models = [
            ...
            if context.request.user.has_perm(f"{app.get('app_label')}.view_{m.get('object_name').lower()}")  # Aquí se verifica el permiso
    ...
    # Si hay menús personalizados en la configuración, añádelos también
    key = 'system_keep'
    if config and 'menus' in config:
        if config.get(key, None):
            ...
            for i in temp:
                if 'permissions' in i:
                    if not context.request.user.has_perms(i['permissions']):
                        continue
                # Procesa las subopciones de menú
                if 'models' in i:
                    for k in i.get('models'):
                        ...
                        if 'permissions' in k:
                            if not context.request.user.has_perms(k['permissions']):
                                continue
                    data.append(i)
        else:
            data = config.get('menus')

    # Ordenar y procesar el menú como en el código original
